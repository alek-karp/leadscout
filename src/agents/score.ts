...
export function scoreLead(
  ...
  const hasContact = enrichment.email.trim().length > 0 || enrichment.phone.trim().length > 0 || clinic.phone.trim().length > 0 || enrichment.contactPage.trim().length > 0;
  ...
  return {
    ...
    phone: enrichment.phone || clinic.phone,
    city: clinic.city,
